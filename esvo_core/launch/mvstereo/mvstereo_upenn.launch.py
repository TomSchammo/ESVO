#!/usr/bin/env python3
"""
ROS2 launch file for ESVO MVStereo with UPenn dataset configuration.
"""
...
def generate_launch_description():
    ...
    calib_dir_arg = DeclareLaunchArgument(
        'calib_dir',
        default_value=os.path.join(esvo_core_share, 'calib', 'upenn'),
    ...
    # MVStereo node
    mvstereo_params_file = os.path.join(esvo_core_share, 'cfg', 'mvstereo', 'mvstereo_upenn.yaml')
    esvo_mvstereo = Node(
        package='esvo_core',
        ...
        remappings=[
            ...
            ('stamped_pose', '/davis/left/pose'),  # UPenn uses different pose topic
        ...
        parameters=[
            ...
            {
                'use_sim_time': True,
                'dvs_frame_id': 'dvs',  # UPenn uses dvs frame
                'world_frame_id': 'map',
