    ...
    depthFramePtr_(new DepthFrame(camSysPtr_->cam_left_ptr_->height_, camSysPtr_->cam_left_ptr_->width_))
{
  ...
  events_right_sub_ = create_subscription<dvs_msgs::msg::EventArray>(
    "events_right", rclcpp::QoS(5000).best_effort(),
  ...
  TS_sync_->registerCallback(std::bind(&esvo_Mapping::timeSurfaceCallback, this, std::placeholders::_1, std::placeholders::_2));
  // TF
  ...
  pc_pub_ = create_publisher<sensor_msgs::msg::PointCloud2>("/esvo_mapping/pointcloud_local", 1);
  if(bVisualizeGlobalPC_)
  {
    gpc_pub_ = create_publisher<sensor_msgs::msg::PointCloud2>("/esvo_mapping/pointcloud_global", 1);
...
void esvo_Mapping::MappingAtTime(const rclcpp::Time& t)
{
  TicToc tt_mapping;
...
bool esvo_Mapping::InitializationAtTime(const rclcpp::Time &t)
{
...
void esvo_Mapping::stampedPoseCallback(const geometry_msgs::msg::PoseStamped::SharedPtr ps_msg)
{
  ...
  tf_buffer_->setTransform(transform_stamped, "esvo_mapping", false);
